  ...
  return (
    <Form action={handleSubmit} className="flex items-center gap-2 flex-col  ">
      ...
      <select
        name="job"
        id="job"
        placeholder=" الوظيفة المتاحة  "
        style={{
          direction: "rtl",
        }}
        className="mt-0.5 flex flex-row-reverse  py-2 px-4 min-w-xs   md:min-w-md rounded bg-white text-black/80 active:text-black text-right border-gray-300 shadow-sm sm:text-sm"
      >
        <option value="web">web design</option>
        <option value="gd">grapihc design</option>
        ...
        <div class="min-w-xs   md:min-w-md py-2 bg-gray-50 rounded border border-gray-300 gap-3 grid border-dashed">
          <div class="grid gap-1">
            ...
            <h2 class="text-center text-gray-400   text-xs leading-4">
              PNG, JPG or PDF, اصغر من 15MB
            </h2>
          </div>
          <div class="grid gap-2">
            <h4 class="text-center text-gray-900 text-sm font-medium leading-snug">
              اسحب وافلت السيرة الذاتية هنا
            </h4>
            <div class="flex items-center justify-center">
              <label>
                <input type="file" id="phone" name="phone" required hidden />
                <div className=" hover:scale-105 active:scale-95 flex w-28 h-9 px-2 flex-col  bg-bgSecondary rounded-full shadow text-white text-xs font-semibold leading-4 items-center justify-center cursor-pointer focus:outline-none">
                  اختيار الملف{" "}
                </div>
              </label>
            </div>
      ...
      >
        {isPending ? (
          ...
          <>
            <span className="tracking-wide "> إرسال الطلب </span>
          </>
        )}{" "}
      </button>
